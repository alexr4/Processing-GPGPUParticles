...
vec2 fbmCurl(in vec3 pos) {
    // Initial values
    vec2 value = vec2(0.0);
    float amplitude = 1.0;//normMouse.x;
    float frequency = 0.5;//normMouse.y;
    //
    // Loop of octaves
    for (int i = 0; i < OCTAVE; i++) {
        value += amplitude * curlNoise(pos).xy;
        pos.xy = rotate2d(PI * 0.01) * pos.xy;
        pos *= 2.5;
        amplitude *= frequency;
    }
    return value;
...
vec2 get2DOffset(sampler2D tex, vec2 texCood){
	...
	if(inputPix.w > 0.95){
		inputPix.z = inputPix.z * -1.0;
	}
	return vec2(-1 * (inputPix.y - inputPix.x), inputPix.z);
	//return vec2(inputPix.x + inputPix.y * -1, inputPix.z);
  //return vec2(inputPix.y - inputPix.x, inputPix.z);
}
...
void main() {
	...
	vec2 origin = (decodeRGBA16(originRGBA)  * 2.0 - 1.0) * edgeVel;
  ...
  vec2 normPos =  loc / worldResolution;
  vec4 ofRGBA =  texture(opticalFlow, vec2(normPos.x, 1.0 - normPos.y));
  vec2 opticalFlowDir = get2DOffset(opticalFlow, vec2(normPos.x, 1.0 - normPos.y));

  //opticalFlowDir = decodeRGBA16(ofRGBA) * 2.0 - 1.0;
  float ofForce = length(opticalFlowDir);

	//define friction
  // vel = vel * 0.995;
  float speed = length(vel) / edgeVel;
	float coeff = 0.05;//mix(0.75, 0.35, speed);
...
/*	float noiseAngle = snoise(vec3(loc * 0.5 * 0.01, time * 0.1)) * PI;
  ...
  vec3 force3D = curlNoise(vec3(loc * vec2(1.0, res) + time, time) * 0.001);
 // vec2 fbmcurl = fbmCurl(vec3(loc, time) *  vec3(0.001) + time * 0.005);
  vec2 noiseForce = force3D.xy;

	//add forces
	//acc += wind/mass;

  acc += (opticalFlowDir * 2000.0)/mass;
	if(length(vel) > 0.01 || random(maxLife) < 0.05){
    acc += (noiseForce / mass);
	  acc += friction;// / mass;
  }
