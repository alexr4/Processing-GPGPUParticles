...
void main(){
	...
	*/
	...
	vec2 normPosition = (pos + (worldResolution - resolution) * 0.5) / worldResolution;
	normPosition = clamp(normPosition, vec2(0), vec2(1.0));
	float mask = texture2D(stencilBuffer, vec2(normPosition.x, 1.0 - normPosition.y)).r;
	float isInside = 1.0 - step(0.5, texture2D(stencilBuffer, vec2(normPosition.x, 1.0 - normPosition.y)).r);

	
	float life = (mod(time, maxLife) / maxLife);

  	float mass = minMass + nmass * (maxMass - minMass);
	float alpha = 1.0 - abs(life * 2.0 - 1.0);

	vec4 clip = projection * modelview * vec4(pos, 0, 1); //we scale the pos to avoid sketch edges
	vec2 weight = vec2(alpha * maxSize * (1.0 - mask));
	...
	vec3 col = vec3(index);
	vertColor = vec4(col, alpha);// * isInside);
}