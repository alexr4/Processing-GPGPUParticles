...
void main() {
	...
	loc += vel;
	vec2 normPosition = loc / worldResolution;
	normPosition = clamp(normPosition, vec2(0), vec2(1.0));
	float isInside = texture2D(stencilBuffer, vec2(normPosition.x, 1.0 - normPosition.y)).r;

	if(isInside > 0.5){
		// loc = vec2(random(loc.xy + time), random(loc.yx + time + 500)) * worldResolution;
	}
	...
	//edge
	loc.x = mod(loc.x, worldResolution.x);
