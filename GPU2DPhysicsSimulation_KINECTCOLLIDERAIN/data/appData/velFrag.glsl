...
void main() {
	...
	float life = (mod(time, maxLife) / maxLife);
...
/*	float noiseAngle = snoise(vec3(loc * 0.5 * 0.01, time * 0.1)) * PI;
  ...
  vec3 force3D = curlNoise(vec3(loc * vec2(1.0, res) + time, time) * 0.0018);
  // vec2 fbmcurl = fbmCurl(vec3(loc, time) *  vec3(vec2(0.001), 0.01));
  vec2 noiseForce = force3D.xy * force3D.z;



	//add forces
	acc += (noiseForce / mass) * 0.05;
	acc += (wind/mass) * 0.5;
	acc += (friction / mass);
	acc += gravity;
  ...
  //collision
  vec2 normalPosition = loc / worldResolution;
  normalPosition = clamp(normalPosition, vec2(0), vec2(1));
  normalPosition = vec2(normalPosition.x, 1.0 - normalPosition.y);

  vec4 normalmap = texture2D(normalBuffer, normalPosition);
  float inside = normalmap.z;
  normalmap.xyz = normalmap.xyz * 2.0 - 1.0;

  vec3 refl = reflect(vec3(vel, 1.0), vec3(normalmap.xy, 0.0));
  //refl.xy = rotate2d(PI * 0.5) * refl.xy;
  vel = refl.xy;
